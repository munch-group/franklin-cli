...
if (-NOT ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-UnlessQuiet  ""
    Write-UnlessQuiet  "Administrator privileges required for Chrome installation" -ForegroundColor Green
    Write-UnlessQuiet  ""
    # Write-UnlessQuiet  "User Password:" -ForegroundColor Green
    Write-UnlessQuiet  "Please approve the Administrator prompt that will appear..." -ForegroundColor Cyan
    Write-UnlessQuiet  ""
...
function Get-ChromeInstallationStatus {
    Write-UnlessQuiet  "=== Google Chrome Installation Status ===" -ForegroundColor Cyan
    ...
    Write-UnlessQuiet  "Chrome Installed: $chromeInstalled" -ForegroundColor $(if ($chromeInstalled) { "Green" } else { "Red" })
    
    if ($chromeInstalled) {
        $version = (Get-Item $chromePath).VersionInfo.FileVersion
        Write-UnlessQuiet  "Version: $version" -ForegroundColor White
        Write-UnlessQuiet  "Location: $chromePath" -ForegroundColor White
    }
    
    # Check Chrome update service
    $updateService = Get-Service -Name "gupdate" -ErrorAction SilentlyContinue
    if ($updateService) {
        Write-UnlessQuiet  "Update Service: $($updateService.Status)" -ForegroundColor White
    } else {
        Write-UnlessQuiet  "Update Service: Not found" -ForegroundColor Red
    }
    ...
    try {
        ...
        if ($defaultBrowser -and $defaultBrowser.ProgId -like "*Chrome*") {
            Write-UnlessQuiet  "Default Browser: Chrome" -ForegroundColor Green
        } else {
            Write-UnlessQuiet  "Default Browser: Not Chrome" -ForegroundColor Yellow
        }
    } catch {
        Write-UnlessQuiet  "Default Browser: Cannot determine" -ForegroundColor Red
    }
    ...
    if (Test-Path $profilePath) {
        ...
        Write-UnlessQuiet  "User Profiles: $($profiles.Count)" -ForegroundColor White
        
        $totalSize = (Get-ChildItem $profilePath -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum
        $sizeGB = [math]::Round($totalSize / 1GB, 2)
        Write-UnlessQuiet  "Profile Data Size: $sizeGB GB" -ForegroundColor White
    } else {
        Write-UnlessQuiet  "User Profiles: None found" -ForegroundColor Red
    }
    ...
    if (Test-Path $policyPath) {
        ...
        Write-UnlessQuiet  "Enterprise Policies: $($policies.Count) configured" -ForegroundColor White
    } else {
        Write-UnlessQuiet  "Enterprise Policies: None configured" -ForegroundColor Yellow
    }
}
...
function Remove-GoogleChrome {
    ...
    Write-UnlessQuiet  "Starting Google Chrome uninstallation..." -ForegroundColor Yellow
    
    try {
        # Stop Chrome processes
        Write-UnlessQuiet  "Stopping Chrome processes..." -ForegroundColor Yellow
        ...
        if ($chromeApp -and $chromeApp.UninstallString) {
            Write-UnlessQuiet  "Running Chrome uninstaller..." -ForegroundColor Yellow
            ...
            Write-UnlessQuiet  "Executing: $uninstallCmd" -ForegroundColor Cyan
        ...
        foreach ($path in $installPaths) {
            if (Test-Path $path) {
                Write-UnlessQuiet  "Removing: $path" -ForegroundColor Yellow
                Remove-Item $path -Recurse -Force -ErrorAction SilentlyContinue
                Write-UnlessQuiet  "Removed: $path" -ForegroundColor Green
            }
        }
        
        # Remove user data if requested
        if (-not $KeepUserData) {
            Write-UnlessQuiet  "Removing user data..." -ForegroundColor Yellow
            ...
            foreach ($user in $users) {
                ...
                if (Test-Path $userChromePath) {
                    ...
                    Write-UnlessQuiet  "Removed user data for: $($user.Name)" -ForegroundColor Green
                }
            }
            ...
            if (Test-Path $currentUserPath) {
                ...
                Write-UnlessQuiet  "Removed current user Chrome data" -ForegroundColor Green
            }
        }
        
        # Remove services
        Write-UnlessQuiet  "Removing Chrome services..." -ForegroundColor Yellow
        ...
        foreach ($serviceName in $services) {
            ...
            if ($service) {
                ...
                Write-UnlessQuiet  "Removed service: $serviceName" -ForegroundColor Green
            }
        }
        
        # Remove registry entries
        Write-UnlessQuiet  "Cleaning registry entries..." -ForegroundColor Yellow
        $registryPaths = @(
        ...
        foreach ($regPath in $registryPaths) {
            if (Test-Path $regPath) {
                ...
                Write-UnlessQuiet  "Removed registry: $regPath" -ForegroundColor Green
            }
        }
        
        # Remove shortcuts
        Write-UnlessQuiet  "Removing shortcuts..." -ForegroundColor Yellow
        $shortcutPaths = @(
        ...
        foreach ($shortcut in $shortcutPaths) {
            if (Test-Path $shortcut) {
                ...
                Write-UnlessQuiet  "Removed shortcut: $shortcut" -ForegroundColor Green
            }
        }
        
        Write-UnlessQuiet  "Google Chrome uninstallation completed!" -ForegroundColor Green
...
function Install-GoogleChrome {
    Write-UnlessQuiet  "Starting Google Chrome installation..." -ForegroundColor Green
    
    # Download Chrome installer
    $installerUrl = "https://dl.google.com/chrome/install/latest/chrome_installer.exe"
    $installerPath = "$env:TEMP\chrome_installer.exe"
    
    Write-UnlessQuiet  "Downloading Chrome installer..." -ForegroundColor Yellow
    try {
    ...
    Write-UnlessQuiet  "Installing Chrome..." -ForegroundColor Yellow
    $installArgs = @("/silent", "/install")
    
    try {
        $process = Start-Process -FilePath $installerPath -ArgumentList $installArgs -Wait -PassThru
        if ($process.ExitCode -eq 0) {
            Write-UnlessQuiet  "Chrome installed successfully" -ForegroundColor Green
        } else {
...
function Set-ChromeConfiguration {
    Write-UnlessQuiet  "Configuring Google Chrome..." -ForegroundColor Yellow
    ...
    if ($DisableUpdates) {
        Write-UnlessQuiet  "Disabling Chrome updates..." -ForegroundColor Yellow
    ...
    if ($DisableTracking) {
        Write-UnlessQuiet  "Configuring privacy settings..." -ForegroundColor Yellow
    ...
    if ($HomepageURL) {
        Write-UnlessQuiet  "Setting homepage to: $HomepageURL" -ForegroundColor Yellow
    ...
    if ($EnterpriseMode) {
        Write-UnlessQuiet  "Configuring enterprise settings..." -ForegroundColor Yellow
    ...
    Write-UnlessQuiet  "Chrome configuration applied" -ForegroundColor Green
}

function Set-ChromeAsDefault {
    Write-UnlessQuiet  "Setting Chrome as default browser..." -ForegroundColor Yellow
    ...
    try {
        ...
        if (Test-Path $chromePath) {
            ...
            Write-UnlessQuiet  "Chrome set as default browser" -ForegroundColor Green
        } else {
...
if ($Uninstall -or $CleanUninstall) {
    Write-UnlessQuiet  "Google Chrome Uninstallation" -ForegroundColor Red
    Write-UnlessQuiet  "=============================" -ForegroundColor Red
    
    # Show current status
    Get-ChromeInstallationStatus
    
    Write-UnlessQuiet  "`nUninstall Options:" -ForegroundColor Yellow
    if ($CleanUninstall) {
        Write-UnlessQuiet  "- Clean uninstall: Removes Chrome and ALL user data (bookmarks, history, etc.)" -ForegroundColor White
    } else {
        Write-UnlessQuiet  "- Standard uninstall: Removes Chrome but keeps user data" -ForegroundColor White
    }
    
    $confirmUninstall = Read-Host "`nProceed with uninstallation? (yes/no)"
    if ($confirmUninstall -ne "yes") {
        Write-UnlessQuiet  "Uninstallation cancelled" -ForegroundColor Yellow
        exit 0
    }
    ...
    if ($result) {
        Write-UnlessQuiet  "`nFinal status check:" -ForegroundColor Cyan
        Get-ChromeInstallationStatus
    }
...
Write-UnlessQuiet  "Google Chrome Installation and Configuration" -ForegroundColor Green
Write-UnlessQuiet  "===========================================" -ForegroundColor Green
...
if ((Test-Path $chromeExe) -or (Test-Path $chromeExe32)) {
    Write-UnlessQuiet  "Chrome is already installed. Applying configuration..." -ForegroundColor Yellow
} else {
    # Install Chrome
...
Write-UnlessQuiet  "`nInstallation and configuration complete!" -ForegroundColor Green
Write-UnlessQuiet  "Usage:" -ForegroundColor Yellow
Write-UnlessQuiet  "- To check status: $($MyInvocation.MyCommand.Name) -StatusCheck" -ForegroundColor White
Write-UnlessQuiet  "- To uninstall: $($MyInvocation.MyCommand.Name) -Uninstall" -ForegroundColor White
Write-UnlessQuiet  "- To clean uninstall: $($MyInvocation.MyCommand.Name) -CleanUninstall" -ForegroundColor White

# Show final status
Write-UnlessQuiet  "`nFinal installation status:" -ForegroundColor Cyan
Get-ChromeInstallationStatus