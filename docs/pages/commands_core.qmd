...
**franklin** uses the [click](https://click.palletsprojects.com/en/stable/) library to define a hierarchy of commands and subcommands. `franklin` is the main command and has the subcommands `download`, `jupyter`, `update`, `show`, and `cleanup`. Should you forget, you can always run the `franklin` command like this:
