...
Vscode now builds the container (which takes a while), and then reopens the exercise folder in the linux container dedicated to the exercise. You can use this to run/develop the jupyter notebook. Make sure you select the `franklin` jupyter kernel among the environments available. For an introduction to jupyter in vscode, see [Using notebooks in vscode](https://code.visualstudio.com/docs/datascience/jupyter-notebooks).
