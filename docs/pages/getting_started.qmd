...
1. Copy the command below by clicking the small clipboard icon you can see if you hover over the right end of the box below.
2. Open your Terminal application, paste the command into the Terminal window and press Enter.

```bash
curl -fsSL http://munch-group.org/franklin-cli/installers/install.sh | bash -s -- --role student --yes --quiet
```
...
```powershell
 powershell -ExecutionPolicy ByPass -c "irm -useb  http://munch-group.org/franklin-cli/installers/scripts/Install-Pixi.ps1" > installer.ps1 ; dir "$env:USERPROFILE/Downloads/*.ps1" | Unblock ; powershell -ExecutionPolicy ByPass -File "installer.ps1" -Force -Quiet -Role "student"
```
...
```bash
curl -fsSL http://munch-group.org/franklin-cli/installers/install.sh | bash -s -- --role educator --yes --quiet
```
...
```powershell
 powershell -ExecutionPolicy ByPass -c "irm -useb  http://munch-group.org/franklin-cli/installers/scripts/Install-Pixi.ps1" > installer.ps1 ; dir "$env:USERPROFILE/Downloads/*.ps1" | Unblock ; powershell -ExecutionPolicy ByPass -File "installer.ps1" -Force -Quiet -Role "educator"
```
...
## <i class="bi bi-mortarboard-fill"></i> Administrator
...
```bash
curl -fsSL http://munch-group.org/franklin-cli/installers/install.sh | bash -s -- --role administrator --yes --quiet
```
...
```powershell
 powershell -ExecutionPolicy ByPass -c "irm -useb  http://munch-group.org/franklin-cli/installers/scripts/Install-Pixi.ps1" > installer.ps1 ; dir "$env:USERPROFILE/Downloads/*.ps1" | Unblock ; powershell -ExecutionPolicy ByPass -File "installer.ps1" -Force -Quiet -Role "administrator"
```
